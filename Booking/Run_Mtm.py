...
def run_Mtm(LS, book_name=None):
    ...
    book.clean_basket()
    MtM = pd.DataFrame(columns=['open position', 'type', 'time to maturity', 'strike', 'quantity', 'asset', 'asset price', 'MtM', 'moneyness %', 'SP', 'pnl', 'opnl', 'delta', 'gamma', 'vega', 'theta'])
    for i in range(len(book.basket)):

        MtM.loc[i] = {'open position':'long' if book.basket[i].position>0 else 'short', 'type': book.basket[i].type, 'time to maturity':book.basket[i].T*365, 'strike':book.basket[i].K, 'quantity': book.basket[i].position, 'asset':book.basket[i].asset.name, 'asset price':book.basket[i].asset.St, 'MtM':book.basket[i].option_price_close_formulae()*LS, 'moneyness %': (book.basket[i].asset.St/book.basket[i].K - 1)*100 if book.basket[i].type=='Call EU' else -(book.basket[i].asset.St/book.basket[i].K - 1)*100, 'SP':None, 'pnl': None, 'opnl':None, 'delta':book.basket[i].Delta_DF(), 'gamma':book.basket[i].Gamma_DF(), 'vega':book.basket[i].Vega_DF(), 'theta': book.basket[i].Theta_DF()}
    ...
    MtM.loc[len(MtM)] = {'open position': 'long' if book.asset.quantity>0 else 'short',
                        ...
                        'SP': sp_book, 'pnl': Mtm_book+sp_book,
                        'opnl': None,
